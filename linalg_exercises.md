...
1. Compute the matrix product $AB$ inner-product-wise and outer-product-wise
    ...
    \end{align*}
    :::
3. You have observations of $5$ symptoms of a disease for three patients represented in the binary matrix 
    $$ A = \begin{pmatrix}
  1 & 0 & 1 & 1 & 0\\
  1 & 1 & 0 & 0 & 0\\
  ...
  Compute the matrix $AA^\top$ and $A^\top A$ and interpret the result with regard to the scenario.
    ...
    :::
4.  Find a matrix/vector notation to compute the vector of average feature values for a matrix $A\in\mathbb{R}^{n\times d}$, representing $n$ observations of $d$ features. Make an example for your computation.
    :::{admonition} Solution.
    :class: dropdown
    ...
    :::
5. Every system of linear equations can be written as a matrix equation $A\vvec{x}=\vvec{y}$. Given the following system of linear equations, what would be the matrix $A$ and vector $\vvec{y}$ such that the system of linear equations is equivalent to solving $A\vvec{x}=\vvec{y}$? 
    \begin{align*}
        2x_1 &+& 3x_2 && &=&4\\
        x_1  &-& 2x_2 &+& x_3 &=& 3\\
        -x_1 &+& 2x_2 &+& 3x_3 &=& 1.
    ...
    ::::
6. Show that $\lVert A - B \rVert^2 = -2\tr(AB^\top) + 2n $ for orthogonal matrices $A,B\in\mathbb{R}^{n\times n}$.
    :::{admonition} See solution.
    :class: dropdown
    ...
    :::
7. Show that the following norms are orthogonal invariant
    * the vector $L_2$-norm
