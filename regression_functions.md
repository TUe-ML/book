...
\end{align*}
where the feature transformation maps now to a $(k+1)^2$-dimensional vector space, $\bm\phi(\vvec{x}),\bm\beta\in\mathbb{R}^{(k+1)^2}$. The basis functions are here the set of $\{x_1^{i_1}x_2^{i_2}\mid 0\leq i_1,i_2\leq k\}$. 
````
...
$$\bm{\phi}_{pk}(\vvec{x}) = (x_1^{i_1}\cdot \ldots \cdot x_d^{i_d})_{0\leq i_1\ldots i_d\leq k} \in\mathbb{R}^{(k+1)^d}, \text{ for }\vvec{x}\in\mathbb{R}^d$$
