...
align: center
---
Optimization in machine learning can be viewed as searching for the lowest valley in a vast and rugged landscape.
````    
...
```` 
In an unconstrained optimization problem, minimizers occur at points where the function cannot be decreased in any direction. In many practical situations, however, we are not free to search over the entire space but need to stay in a feasible region defined by constraints. This leads to a *constrained optimization problem*.
...
Boundary minimizers can be difficult to identify because finding minimizers of a (smooth) function is more easy than finding minimizers of a function and having to check all points on the boundary of the feasible set. Luckily, there is a way to transform a constrained objective into an (almost) unconstrained one, that works for many function classes. This is done by the Lagrangian function.
